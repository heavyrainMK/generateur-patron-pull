...
function telechargerPDF() {
    ...
    if (!resultatDiv || !resultatDiv.textContent.trim()) {
        alert("Aucun patron à télécharger !");
        return;
    }

    // Préparation du contenu
    const pdfDiv = document.getElementById('pdf-patron');
    pdfDiv.innerHTML = `
        <div style="
            ...
            width: 600px;
            border-radius: 14px;
            box-shadow: 0 6px 22px rgba(90,50,110,0.07);
            ">
            <div style="text-align:center; margin-bottom: 18px;">
                <img src="https://cdn-icons-png.flaticon.com/128/7076/7076251.png" alt="Pelote de laine" width="48" style="margin-bottom: 6px;"/>
                <span style="font-size: 2.2rem; font-weight: 800; color: #694ED6;">Patron de Pull Sur-Mesure</span>
                <div style="margin-top: 8px; font-size: 1.1rem; color: #6b7280;">Généré par le Générateur de Patrons</div>
            </div>
            <div style="margin: 22px 0; font-size: 1.05rem; line-height: 1.65; background: #f6f8fc; border-radius: 10px; padding: 16px 18px;">
                ${formatPatronPourPDF(resultatDiv.textContent)}
            </div>
            <div style="font-size:0.95rem; color: #9ca3af; text-align:center; margin-top: 40px;">
                Patron généré le ${new Date().toLocaleDateString()} &bull; Projet étudiant L2 &copy; 2025
            </div>
        </div>
    `;

    // On utilise jsPDF avec html2canvas pour rendre tout le HTML stylé
    ...
    doc.html(pdfDiv, {
        callback: function (doc) {
            doc.save("patron_pull_sur_mesure.pdf");
        },
        margin: [24, 24, 24, 24],
        autoPaging: "text",
        x: 12,
...
// Transforme le texte brut du patron en HTML lisible
function formatPatronPourPDF(texte) {
    if (!texte) return "";
    let html = texte
        .replace(/\n\s*\n/g, '</p><p>')           // paragraphes
        .replace(/^- (.+)$/gm, '<li>$1</li>')     // listes à puces
        .replace(/\n/g, '<br>');                  // sauts de ligne

    // Si on a au moins une puce, entoure de <ul>
    if (html.includes('<li>')) {
        html = html.replace(/(<li>[\s\S]*<\/li>)/g, '<ul style="margin-top:10px;">$1</ul>');
    }
    return '<p>' + html + '</p>';
}
